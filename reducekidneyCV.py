...
# Function to create balanced evaluation set:
# - All negative samples (class 0)
# - Twice as many positive samples (class 1)
def create_balanced_eval_set(X, y):
    """
    Creates a balanced evaluation set with:
    - All negative samples (class 0)
    - Twice as many positive samples (class 1) randomly selected
    ...
    """
    # Get indices of negative and positive samples
    ...
    # Select twice as many positive samples (or all if not enough)
    n_positives_to_select = min(2 * n_negatives, len(pos_indices))
    selected_pos_indices = np.random.choice(pos_indices, size=n_positives_to_select, replace=False)
    
    # Combine indices
    eval_indices = np.concatenate([neg_indices, selected_pos_indices])
    
    # Return the selected subset
    return X[eval_indices], y[eval_indices]
...
# Create balanced evaluation set as specified:
# - All negative samples (class 0)
# - Twice as many positive samples (class 1)
X_eval, y_eval = create_balanced_eval_set(X_array, y_array)

print(f"\nCreated balanced evaluation set:")
print(f"Total samples: {len(y_eval)}")
print(f"Class 0 (negative): {np.sum(y_eval == 0)} samples")
print(f"Class 1 (positive): {np.sum(y_eval == 1)} samples")
print(f"Ratio positive:negative = {np.sum(y_eval == 1) / np.sum(y_eval == 0):.2f}")

# Set up 5-fold cross-validation
kf = KFold(n_splits=5, shuffle=True, random_state=42)

# 1. Random Forest with 5-fold CV on balanced set
print("\n1. Running Random Forest with 5-fold CV...")
rf_model = RandomForestClassifier(random_state=42)
rf_scores = cross_val_score(rf_model, X_eval, y_eval, cv=kf, scoring='accuracy')
...
# 2. XGBoost with 5-fold CV on balanced set
print("\n2. Running XGBoost with 5-fold CV...")
xgb_model = xgb.XGBClassifier(random_state=42)
xgb_scores = cross_val_score(xgb_model, X_eval, y_eval, cv=kf, scoring='accuracy')
...
# 3. Mean-Shift with 5-fold CV on balanced set
print("\n3. Running Mean-Shift with 5-fold CV...")
meanshift_scores = []

for fold_idx, (train_idx, test_idx) in enumerate(kf.split(X_eval)):
...
# 4. Mean-Shift on full balanced evaluation set
print("\n4. Running Mean-Shift on full balanced evaluation set...")

# Calculate bandwidth for full balanced evaluation set
bandwidth_full = np.mean(np.std(X_eval, axis=0)) * 0.5
print(f"Using bandwidth: {bandwidth_full:.4f}")

# Apply Mean-Shift clustering on full balanced evaluation set
meanshift_full = MeanShift(bandwidth=bandwidth_full, bin_seeding=True)
cluster_labels_full = meanshift_full.fit_predict(X_eval)

# Count clusters in full balanced evaluation set
n_clusters_full = len(np.unique(cluster_labels_full))
print(f"Number of clusters found: {n_clusters_full}")

# Map clusters to binary predictions for full balanced evaluation set
y_pred_full = map_clusters_to_binary(cluster_labels_full, y_eval)

# Calculate final accuracy
final_acc = accuracy_score(y_eval, y_pred_full)
print(f"Full balanced evaluation set accuracy: {final_acc:.4f}")

# Print confusion matrix and classification report
print("\nFull Balanced Evaluation Set Confusion Matrix:")
print(confusion_matrix(y_eval, y_pred_full))
print("\nClassification Report:")
print(classification_report(y_eval, y_pred_full))
...
# Results Summary
print("\n--- Results Summary (Balanced Evaluation Set) ---")
